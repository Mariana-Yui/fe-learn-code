...
import { minify, Options as MiniOptions } from 'html-minifier-terser';
...
class NonstandardWebpackPlugin {
  ...
  apply(compiler: Compiler) {
    ...
    compiler.hooks.compilation.tap(pluginName, (compilation) => {
      ...
      hook.tapPromise(pluginName, async (data) => {
        let apreHtml: string = data.html;
        this.options.replace.forEach(({ from, to }) => {
          apreHtml = apreHtml.replace(from, to);
        });

        this.apreHtml = await minify(apreHtml, this.options.minify as MiniOptions);
        return data;
      });
    });
  ...
  async hookWithZip(compiler: Compiler) {
